...
def sample_images(batches_done, G_AB, G_BA, val_dataloader, opt, Tensor):
    """保存生成的样本图像"""
...
def train(opt):
    ...
    if opt['epoch'] != 0:
        ...
        G_AB.load_state_dict(torch.load("checkpoints/%s/G_AB_%d.pth" % (opt['dataset_name'], opt['epoch'])))
        G_BA.load_state_dict(torch.load("checkpoints/%s/G_BA_%d.pth" % (opt['dataset_name'], opt['epoch'])))
        D_A.load_state_dict(torch.load("checkpoints/%s/D_A_%d.pth" % (opt['dataset_name'], opt['epoch'])))
        D_B.load_state_dict(torch.load("checkpoints/%s/D_B_%d.pth" % (opt['dataset_name'], opt['epoch'])))
    else:
        # Initialize weights
    ...
    for epoch in range(opt['epoch'], opt['n_epochs']):
        for i, batch in enumerate(dataloader):
            ...
            if batches_done % opt['sample_interval'] == 0:
                sample_images(batches_done, G_AB, G_BA, val_dataloader, opt, Tensor)

        if opt['checkpoint_interval'] != -1 and (epoch + 1) % opt['checkpoint_interval'] == 0:
            # Save model checkpoints
            torch.save(G_AB.state_dict(), "checkpoints/%s/G_AB_%d.pth" % (opt['dataset_name'], epoch))
            torch.save(G_BA.state_dict(), "checkpoints/%s/G_BA_%d.pth" % (opt['dataset_name'], epoch))
            torch.save(D_A.state_dict(), "checkpoints/%s/D_A_%d.pth" % (opt['dataset_name'], epoch))
            torch.save(D_B.state_dict(), "checkpoints/%s/D_B_%d.pth" % (opt['dataset_name'], epoch))

            print("Saved models at epoch %d" % (epoch))
